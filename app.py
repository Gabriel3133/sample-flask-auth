...
def login():
    ...
    if username and password:
        ...
        if user and bcrypt.checkpw(str.encode(password), str.encode(user.password)):
                ...
                return jsonify( {"message": "Authentication completed successfully"}), 200
...
def create_user():
    ...
    return jsonify( {"message": "Ivalid data"}), 400
...
def delete_user(id_user):
    ...
    if current_user.role != "admin":
            return jsonify( {"message": "Operetion not allowed"}), 403
